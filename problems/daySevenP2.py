...
def dame_rango(mano: str):
    ...
    if 'J' in conteo_cartas.keys(): #tiene jotas
        if len(conteo_cartas) < 5 and len(conteo_cartas) != 1: #tiene al menos una pareja
            clave_mas_repetida = list(conteo_cartas.keys())[0]
            segundo_valor = list(conteo_cartas.values())[1]
            if clave_mas_repetida == 'J' and segundo_valor == 1:
                mayor = 0
                for carta in conteo_cartas.keys():
                    if dame_valor_carta(carta) > mayor:
                        carta_mas_alta = carta
                        mayor = dame_valor_carta(carta)
                conteo_cartas[carta_mas_alta] += conteo_cartas['J']
                conteo_cartas = dict(sorted(conteo_cartas.items(), key=lambda x: x[1], reverse=True))
            elif clave_mas_repetida == 'J' and segundo_valor == 2:
                conteo_cartas[list(conteo_cartas.keys())[1]] += conteo_cartas['J']
            else:
                conteo_cartas[clave_mas_repetida] += conteo_cartas['J']
            conteo_cartas.pop('J')
        elif conteo_cartas['J'] == 1 and len(conteo_cartas) == 5:
            mayor = 0
            for carta in conteo_cartas.keys():
                if dame_valor_carta(carta) > mayor:
                    carta_mas_alta = carta
